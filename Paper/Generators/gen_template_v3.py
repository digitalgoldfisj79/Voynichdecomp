#!/usr/bin/env python3
"""
Generator 10: Templates + Ductus + Corrected Suffix Rule

SAME 6 RULES with Rule 2 CORRECTED from f57v evidence:
  Old Rule 2: Always change suffix (0% repeat)
  New Rule 2: Usually change suffix (87% change, 13% repeat)
...
def main():
    print("=" * 70)
    print("GENERATOR 10: TEMPLATES + DUCTUS + CORRECTED SUFFIX")
    print("=" * 70)
    print("""
    SAME 6 RULES + CORRECTED RULE 2:
      A) Template sampling (whole P70-C entries)
      B) Ductus filter (valid character bigrams)
      C) Suffix change rate: 87% (from f57v Lines 2,4), not 100%
    """)
    ...
    with open('results/template_v3_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('TMPL+DUCT+SFX', s15['n_pass'], s85['n_pass']),
    ...
    with open('results/template_v3_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_v3_scores.pkl")
