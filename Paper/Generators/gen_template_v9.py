...
def build_spec(p70c_path='data/p70c_full_spec_v1.json',
    ...
    # RULE 7: Section default words — the scribe's habitual rest stroke.
    # Each section's #1 most frequent word. All are minimal forms.
    spec['section_defaults'] = {
        'Astronomical': 'ar',
        'Balneological': 'shedy',
        'Cosmological': 'ar',
        'Herbal-A': 'daiin',
        'Herbal-B': 'daiin',
        'Pharmaceutical': 'daiin',
        'Rosettes': 'aiin',
        'Stars': 'aiin',
        'Zodiac': 'ar',
    }
...
class TemplateScribe:
    ...
    def __init__(self, spec, section='Herbal-A', seed=42):
        ...
        # Rule 7: section default word
        self.default_word = spec.get('section_defaults', {}).get(section, None)
        # Pre-computed PGCS slots for each default (all minimal forms)
        self._default_slots = {
            'daiin': ('d', '∅', '∅', 'N'),
            'aiin':  ('∅', '∅', '∅', 'N'),
            'ar':    ('∅', '∅', '∅', 'R'),
        ...
        self.default_slots = self._default_slots.get(self.default_word)
    ...
    def write_section(self, n_tokens, tokens_per_line=10):
        """Same production process. Rules 1-7."""
        corpus = []
        slots_history = []
        line_pos = 0
        ...
        for i in range(n_tokens):
            if line_pos == 0:
                # Rule 7: section default at line start (~7%)
                if self.default_word and self.rng.random() < 0.07:
                ...
                # Rule 7: section default at fresh-word point (~7%)
                if self.default_word and self.rng.random() < 0.07:
                    word = self.default_word
                    slots = self.default_slots
                else:
...
def main():
    print("=" * 70)
    print("GENERATOR 16: + RULE 7 DEFAULT WORD")
    print("=" * 70)
    print("""
    ...
    with open('results/template_v9_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('+DEFAULT', s15['n_pass'], s85['n_pass']),
        ('Gen 13 (cooccur)', 12, 70),
    ...
    with open('results/template_v9_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_v9_scores.pkl")
