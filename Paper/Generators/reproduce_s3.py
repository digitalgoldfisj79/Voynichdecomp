...
=====================================================
Reproduces all generator scores, ablation results, and impossibility
diagnostics reported in Supplement S3.

Generators: 21 original (6 BG22 + 15 template/zero-corpus) plus
  Gen-TS v8b   — two-stream generative with token-length coupling
  Gen-Avoid    — per-triple all-history avoidance (transcription model)
  Gen-Avoid also runs cross-section validation (9 sections).
...
Data dependencies (auto-located):
    ...
    - voynich_transcriptions_slim.json  (transcription, for zero-corpus generators only)
    - transition_lookup.json    (prefix transitions, for two-stream generators only)
...
Generator files (in Paper/Generators/):
    ...
    gen_scribal_p70c.py, gen_ts_v8b.py, gen_transcription_avoid.py

Also requires (in Paper/):
    score_85_metrics.py      (scoring module)
    reproduce_all.py         (BG22 inline generators + helper functions)
...
GENERATOR_REGISTRY = {
    ...
    # Two-stream generative family (1)
    'Gen-TS':  {'path': 'gen_ts_v8b.py',             'family': 'Two-Stream',
                'desc': 'Two-stream + length coupling','interface': 'two_stream'},
    # Transcription model family (1)
    'Gen-Avoid':{'path': 'gen_transcription_avoid.py','family': 'Transcription',
                'desc': 'Per-triple all-history avoid','interface': 'transcription'},
}
...
def find_data_files(root):
    ...
    # voynich_transcriptions_slim.json (optional — zero-corpus generators only)
    for p in [root / 'voynich_transcriptions_slim.json',
              root / 'data' / 'voynich_transcriptions_slim.json',
              root.parent / 'voynich_transcriptions_slim.json',
              root.parent / 'data' / 'voynich_transcriptions_slim.json',
              # Legacy name
              root / 'slim.json',
    ...
    # transition_lookup.json (optional — two-stream generators only)
    for p in [root / 'transition_lookup.json',
              root / 'data' / 'transition_lookup.json',
              root.parent / 'Paper' / 'transition_lookup.json']:
        if p.exists():
            files['transitions'] = p
            break
...
# ── Sections for cross-section validation ──
VMS_SECTIONS = [
...
def run_two_stream_generator(gen_path, n_seeds, n_target,
                              compute_metrics_fn, vms_baseline, tolerances,
                              records_path=None, p70c_path=None,
                              transitions_path=None):
    """
    Run Gen-TS v8b (two-stream generative with token-length coupling).

    This generator builds the triple stream from scratch using:
    - Two lexical regimes (63 EC vs 3,037 FC triples)
    - Preferential reuse (monotonic with past frequency)
    - Boundary-biased innovation (FIRST/LAST enriched)
    - Suffix mutation on reuse (~50-55%)
    - Token-length coupling (AC1 mechanism)
    """
    spec_mod = importlib.util.spec_from_file_location("gen_mod", gen_path)
    mod = importlib.util.module_from_spec(spec_mod)
    spec_mod.loader.exec_module(mod)

    # Extract parameters (needs records, spec, transitions)
    params, records = mod.extract_parameters(
        str(records_path), str(p70c_path), str(transitions_path))

    all_metrics, all_imp, all_scores = [], [], []

    for seed_offset in range(n_seeds):
        seed = SEED + seed_offset
        gen = mod.GenTS_v8b(params, seed=seed)
        corpus, gen_lines = gen.generate()
        corpus = corpus[:n_target]
...
def run_transcription_generator(gen_path, n_seeds,
                                 compute_metrics_fn, vms_baseline, tolerances,
                                 records_path=None):
    """
    Run Gen-Avoid (per-triple all-history avoidance transcription model).

    This model takes the VMS triple stream as input and selects suffix
    variants via frequency-weighted sampling with binary avoidance penalty.

    Runs against Stars section for headline score, then cross-section
    validation against all 9 sections.
    """
    spec_mod = importlib.util.spec_from_file_location("gen_mod", gen_path)
    mod = importlib.util.module_from_spec(spec_mod)
    spec_mod.loader.exec_module(mod)

    with open(str(records_path), 'rb') as f:
        records = pickle.load(f)

    # ── Stars headline score (comparable to other generators) ──
    all_metrics, all_imp, all_scores = [], [], []
    stars_recs = [r for r in records if r['section'] == 'Stars']
    stars_lines, stars_line_triples, stars_menus, _ = mod.build_section_data(
        records, 'Stars')
    vms_stars_tokens = [r['token'] for r in stars_recs]
    vms_stars_metrics = compute_metrics_fn(vms_stars_tokens, lines=stars_lines, seed=42)

    for seed_offset in range(n_seeds):
        seed = SEED + seed_offset
        gen_lines, gen_flat = mod.generate_avoid(
            stars_line_triples, stars_menus, seed=seed, penalty=0.1)
        metrics = compute_metrics_fn(gen_flat, lines=gen_lines, seed=seed)
        imp = compute_impossibility_metrics(gen_flat)
        s84 = score_84(metrics, vms_stars_metrics, tolerances)
        all_metrics.append(metrics)
        all_imp.append(imp)
        all_scores.append(s84['n_pass'])
    ...
    cross_section = {}
    for section in VMS_SECTIONS:
        sec_lines, sec_triples, sec_menus, sec_recs = mod.build_section_data(
            records, section)
        vms_sec_tokens = [r['token'] for r in sec_recs]
        vms_sec_metrics = compute_metrics_fn(
            vms_sec_tokens, lines=sec_lines, seed=42)

        sec_scores = []
        for seed_offset in range(n_seeds):
            seed = SEED + seed_offset
            gen_lines_s, gen_flat_s = mod.generate_avoid(
                sec_triples, sec_menus, seed=seed, penalty=0.1)
            gen_sec_metrics = compute_metrics_fn(
                gen_flat_s, lines=gen_lines_s, seed=seed)
            s84 = score_84(gen_sec_metrics, vms_sec_metrics, tolerances)
            sec_scores.append(s84['n_pass'])

        vms_types = len(set(vms_sec_tokens))
        # Quick type recovery check (seed 42 only)
        _, gen_check = mod.generate_avoid(sec_triples, sec_menus, seed=42, penalty=0.1)
        gen_types = len(set(gen_check))

        cross_section[section] = {
            'scores_84': sec_scores,
            'median_score': int(np.median(sec_scores)),
            'vms_types': vms_types,
            'gen_types': gen_types,
            'type_recovery': gen_types / vms_types if vms_types > 0 else 0,
            'n_tokens': len(vms_sec_tokens),
        }
...
def write_summary(all_results, ablation, ceiling, results_dir):
    ...
    # Ranked table
    lines.append("## All Generators (ranked)")
    ...
    # Cross-section validation (Gen-Avoid)
    avoid_result = all_results.get('Gen-Avoid')
    if avoid_result and 'cross_section' in avoid_result:
        lines.append("")
        lines.append("## Cross-Section Validation (Gen-Avoid)")
        lines.append(f"{'Section':<18s}  {'Score':>7s}  {'Types':>16s}  {'Recovery':>9s}")
        lines.append("-" * 55)
        cs = avoid_result['cross_section']
        all_medians = []
        for sec in VMS_SECTIONS:
            if sec in cs:
                d = cs[sec]
                lines.append(
                    f"  {sec:<18s}  {d['median_score']:3d}/84  "
                    f"{d['gen_types']:>5d}/{d['vms_types']:<5d}  "
                    f"{d['type_recovery']:.1%}")
                all_medians.append(d['median_score'])
        if all_medians:
            mean_s = np.mean(all_medians)
            lines.append(f"\n  Mean: {mean_s:.1f}/84  "
                         f"Range: {min(all_medians)}–{max(all_medians)}")
...
def main():
    ...
    if not args.skip_template:
        ...
        for paper_name, info in GENERATOR_REGISTRY.items():
            ...
            # Zero-corpus generators need slim.json
            if info['interface'] in ('f57v', 'scribal') and not has_slim:
                print(f"  {paper_name}: SKIP (voynich_transcriptions_slim.json not found)")
                continue

            # Two-stream generators need transition_lookup.json
            if info['interface'] == 'two_stream' and 'transitions' not in data_files:
                print(f"  {paper_name}: SKIP (transition_lookup.json not found)")
                continue
            ...
            try:
                ...
                elif info['interface'] == 'two_stream':
                    result = run_two_stream_generator(
                        gen_path, N_SEEDS, N_TARGET,
                        compute_metrics, vms_baseline, TOLERANCES,
                        records_path=data_files['records'],
                        p70c_path=data_files['p70c'],
                        transitions_path=data_files['transitions'])
                elif info['interface'] == 'transcription':
                    result = run_transcription_generator(
                        gen_path, N_SEEDS,
                        compute_metrics, vms_baseline, TOLERANCES,
                        records_path=data_files['records'])
    ...
    print("=" * 70)
    print(f"  Generators scored: {len(all_results)}")
    print(f"  Results in: {results_dir}/")
    print(f"  Files:")
    for f in sorted(results_dir.glob('s3_*.pkl')):
        print(f"    {f.name}")

    # Print cross-section validation if available
    avoid = all_results.get('Gen-Avoid')
    if avoid and 'cross_section' in avoid:
        print(f"\n  Gen-Avoid cross-section validation:")
        cs = avoid['cross_section']
        for sec in VMS_SECTIONS:
            if sec in cs:
                d = cs[sec]
                print(f"    {sec:<18s}  {d['median_score']:3d}/84  "
                      f"Types={d['gen_types']}/{d['vms_types']} "
                      f"({d['type_recovery']:.1%})")
        medians = [cs[s]['median_score'] for s in VMS_SECTIONS if s in cs]
        if medians:
            print(f"    Mean: {np.mean(medians):.1f}/84  "
                  f"Range: {min(medians)}–{max(medians)}")
