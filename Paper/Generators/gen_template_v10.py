...
class TemplateScribe:
    ...
    def mutate_word(self, template_slots, prev_sfx_fam='Y'):
        ...
        for attempt in range(20):
            ...
            # CO-OCCURRENCE 1: gallows demand completion.
            ...
            # CO-OCCURRENCE 2: minimal forms demand completion.
            # VMS: BARE minimals (P+∅+∅+BARE) = 2.6% of corpus.
            # Generator produces 16.9% words ≤2 chars without this.
            # A scribe doesn't just write a prefix and stop.
            is_minimal = (g in ('∅', '')) and (c in ('∅', ''))
            if is_minimal and new_sf == 'BARE':
                resample_tries = 0
                while new_sf == 'BARE' and resample_tries < 5:
                    new_sf = self._pick('sfx_fam')
                    resample_tries += 1
...
def main():
    print("=" * 70)
    print("GENERATOR 17: + MINIMAL COMPLETION")
    print("=" * 70)
    print("""
    CORRECTED RULES + MINIMAL FORM PRESERVATION:
      ...
      E) Gallows reject BARE suffix
      F) Minimal forms reject BARE suffix
    """)
    ...
    with open('results/template_v10_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('+MINCOMP', s15['n_pass'], s85['n_pass']),
        ('Gen 13 (gallows)', 12, 70),
    ...
    with open('results/template_v10_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_v10_scores.pkl")
