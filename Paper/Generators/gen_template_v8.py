#!/usr/bin/env python3
"""
Generator 11: Templates + Ductus + Corrected Suffix + Minimal Preservation

SAME RULES with two corrections:
  Rule 2 CORRECTED: 87% suffix change (13% repeat, from f57v Lines 2,4)
  Rule 3 CORRECTED: Minimal forms (∅G+∅C) only change prefix.
  A scribe doesn't insert a tall letter into a short word when copying.

Plus two principled additions:
...
def build_spec(p70c_path='data/p70c_full_spec_v1.json',
    ...
    # ROUNDED SECTION GALLOWS: nearest 5%, adjusted to sum to 100
    # Pattern a scribe can remember: multiples of 5 only
    rounded_gal = {
        'Astronomical':   {'∅': 60, 'k': 20, 't': 10, 'p': 5, '_cmpd': 5},
        'Balneological':  {'∅': 60, 'k': 25, 't': 10, 'p': 0, '_cmpd': 5},
        'Cosmological':   {'∅': 60, 'k': 20, 't': 15, 'p': 0, '_cmpd': 5},
        'Herbal-A':       {'∅': 60, 'k': 15, 't': 10, 'p': 5, '_cmpd': 10},
        'Herbal-B':       {'∅': 55, 'k': 20, 't': 15, 'p': 5, '_cmpd': 5},
        'Pharmaceutical': {'∅': 65, 'k': 20, 't': 10, 'p': 0, '_cmpd': 5},
        'Rosettes':       {'∅': 55, 'k': 15, 't': 20, 'p': 5, '_cmpd': 5},
        'Stars':          {'∅': 55, 'k': 20, 't': 15, 'p': 5, '_cmpd': 5},
        'Zodiac':         {'∅': 50, 'k': 20, 't': 25, 'p': 0, '_cmpd': 5},
    }
    compound_types = ['cth', 'ckh', 'cph', 'cfh', 'f']
    for sect, rnd in rounded_gal.items():
        if sect not in spec['section_gallows']:
            continue
        orig = section_gallows[sect]
        # Distribute compound weight proportionally
        cmpd_total = sum(orig.get(g, 0) for g in compound_types)
        new_dist = {'∅': rnd['∅'], 'k': rnd['k'], 't': rnd['t'], 'p': rnd['p']}
        for g in compound_types:
            if cmpd_total > 0:
                new_dist[g] = rnd['_cmpd'] * orig.get(g, 0) / cmpd_total
            else:
                new_dist[g] = rnd['_cmpd'] / len(compound_types)
        # Remove zero entries
        new_dist = {k: v for k, v in new_dist.items() if v > 0}
        spec['section_gallows'][sect] = new_dist
...
class TemplateScribe:
    ...
    def mutate_word(self, template_slots, prev_sfx_fam='Y'):
        """
        RULE 2 (CORRECTED): Change suffix ~87% of the time (13% repeat).
        Evidence: f57v Lines 2,4 show 12-13% consecutive suffix repetition.
        ...
        for attempt in range(20):
            # RULE 2 CORRECTED: 87% change suffix, 13% keep it
            # (derived from f57v output rings, not corpus-fitted)
            if self.rng.random() < 0.87:
                ...
                new_sf = sf  # keep same suffix family

            # CO-OCCURRENCE: gallows demand completion.
            # VMS: gallows+BARE = 7.9%, ∅-gallows+BARE = 12.6%.
            # During mutation, if word has gallows and we picked BARE,
            # resample. Templates that START as gallows+BARE are fine.
            # A scribe doesn't write a tall letter and then stop.
            if g not in ('∅', '') and new_sf == 'BARE':
                resample_tries = 0
                while new_sf == 'BARE' and resample_tries < 5:
                    new_sf = self._pick('sfx_fam')
                    resample_tries += 1
            ...
            # Rule 3: half the time change one more slot
            # Rule 3 CORRECTED: Always change at least one other slot.
            # VMS shows 96.2% of consecutive pairs change a non-suffix slot.
            # 3.8% remainder is consistent with scribal error.
            # MINIMAL FORM PRESERVATION: if ∅G+∅C, only change prefix.
            new_p, new_g, new_c = p, g, c
            is_minimal = (g in ('∅', '')) and (c in ('∅', ''))
            if is_minimal:
                # Minimal form: only prefix can change
                new_p = self._pick_prefix_transition(prev_sfx_fam)
            else:
                # Full form: any slot can change
...
def main():
    print("=" * 70)
    print("GENERATOR 15: ROUNDED GALLOWS WEIGHTS")
    print("=" * 70)
    print("""
    CORRECTED RULES + MINIMAL FORM PRESERVATION:
      A) Template sampling (whole P70-C entries)
      B) Ductus filter (valid character bigrams)
      C) Suffix change rate: 87% (from f57v Lines 2,4)
      D) Always change one other slot
      E) Gallows words reject BARE suffix during mutation
    """)
    ...
    with open('results/template_v8_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('RND-GAL', s15['n_pass'], s85['n_pass']),
        ('Gen 13 (precise)', 12, 70),
        ('Template+Ductus', 11, 70),
        ('Section', 10, 67),
        ('Combined', 10, 66),
        ('P70C', 9, 64),
        ('Bigram', 8, 50),
    ...
    with open('results/template_v8_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_v8_scores.pkl")
