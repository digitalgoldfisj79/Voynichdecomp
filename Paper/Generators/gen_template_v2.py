...
class TemplateScribe:
    ...
    def mutate_word(self, template_slots, prev_sfx_fam='Y'):
        ...
        for attempt in range(20):
            ...
            # Rule 3: half the time change one more slot
            # Core LOCKED: scribe preserves the word's backbone
            new_p, new_g, new_c = p, g, c
            if self.rng.random() < 0.50:
                slot = self.rng.choice(['prefix', 'gallows'])
                if slot == 'prefix':
...
def main():
    print("=" * 70)
    print("GENERATOR 9b: TEMPLATES + DUCTUS + CORE-LOCKED")
    print("=" * 70)
    print("""
    ...
    with open('results/template_ductus_v2_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('TMPL+DUCTUS v2', s15['n_pass'], s85['n_pass']),
        ('SM + P70-C', 11, 69),
        ('Section', 10, 67),
        ('Combined', 10, 66),
        ('P70C', 9, 64),
    ...
    for name, c, f in rows:
        marker = ' ◄' if name == 'TMPL+DUCTUS v2' else ''
    ...
    with open('results/template_ductus_v2_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_ductus_v2_scores.pkl")
