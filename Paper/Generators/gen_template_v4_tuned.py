#!/usr/bin/env python3
"""
Generator 11-TUNED: Same rules, corpus-tuned core + suffix vocabulary

IDENTICAL rules and architecture to Gen 11. ONLY CHANGE:
  - Core slot: weighted by VMS corpus frequency (not P70-C ledger)
  - Suffix: sampled as actual strings at corpus frequency (not family→expand)
  
This isolates the claim: the structural model is correct,
only the vocabulary richness was missing.
"""
...
def build_spec(p70c_path='data/p70c_full_spec_v1.json',
    ...
    # ── CORPUS-TUNED: suffix strings per family from actual VMS ──
    corpus_sfx_by_fam = defaultdict(Counter)
    for r in records:
        corpus_sfx_by_fam[r['sfx_fam']][r['suffix']] += 1
    spec['corpus_sfx_by_fam'] = {f: dict(c) for f, c in corpus_sfx_by_fam.items()}

    # ── CORPUS-TUNED: core distribution from actual VMS tokens ──
    corpus_core = Counter(r['m_core'] for r in records)
    spec['corpus_core'] = dict(corpus_core)

    # ── CORPUS-TUNED: suffix as actual strings, not family→expand ──
    corpus_suffix_str = Counter(r['suffix'] for r in records)
    spec['corpus_suffix_str'] = dict(corpus_suffix_str)
...
class TemplateScribe:
    ...
    def __init__(self, spec, section='Herbal-A', seed=42):
        ...
        # Prefix: from P70-C (unchanged)
        d = spec['prefix']
        self.slot_opts['prefix'] = list(d.keys())
        self.slot_wts['prefix'] = [d[k] for k in d]

        # CORPUS-TUNED: core from actual VMS tokens
        d = spec['corpus_core']
        self.slot_opts['core'] = list(d.keys())
        self.slot_wts['core'] = [d[k] for k in d]
        ...
        # CORPUS-TUNED: suffix strings from actual VMS per family
        self.suffix_expand = spec['corpus_sfx_by_fam']
    ...
    def mutate_word(self, template_slots, prev_sfx_fam='Y'):
        ...
        for attempt in range(20):
            ...
            # Rule 3: half the time change one more slot
            # MINIMAL FORM PRESERVATION: if template has ∅ gallows + ∅ core,
            # only change prefix. Don't add structure that wasn't there.
            # A scribe doesn't insert a tall letter into a short word.
...
def main():
    print("=" * 70)
    print("GENERATOR 11-TUNED: CORPUS-TUNED CORE + SUFFIX")
    print("=" * 70)
    print("""
    IDENTICAL RULES to Gen 11. Only two things change:
      - Core slot: VMS corpus frequency (not P70-C ledger)
      - Suffix strings: VMS corpus frequency (not family→expand)
    
    This isolates the claim: the PROCESS is correct,
    only the VOCABULARY was limited.
    """)
    ...
    with open('results/template_v4_tuned_corpora.pkl', 'wb') as f:
    ...
    print("-" * 40)
    rows = [
        ('TUNED', s15['n_pass'], s85['n_pass']),
        ('Gen 11 (unfitted)', 13, 72),
        ('Tmpl+Duct+Sfx', 12, 70),
        ('Section', 10, 67),
        ('Combined', 10, 66),
        ('P70C', 9, 64),
    ...
    with open('results/template_v4_tuned_scores.pkl', 'wb') as f:
        pickle.dump({
    ...
    print(f"\nSaved results/template_v4_tuned_scores.pkl")
