...
The suite contains 84 distributional metrics derived from the full 90-metric computation by excluding 6 Levenshtein-distance subsampling metrics from the headline scores:

> **Reported separately (6):** `wordunique_mean`, `wordunique_std`, `wordunique_skew`, `wordchange_mean`, `wordchange_std`, `wordchange_skew`

These 6 are computed by the BG22 subsampling procedure (100 iterations × 200-word samples) using edit-distance calculations. They are excluded from the headline /84 scores to maintain comparability across scoring sessions, but all values are computed and reported in §S3.3a below. The full Bowern-Gaskell 42-metric benchmark is therefore available for all 23 generators.

The 84 metrics divide as:

| Category | Count | Source |
|---|---|---|
| BG22 benchmark | 36 | Gaskell & Bowern (2022), 42 minus 6 Levenshtein |
| Structural (PGCS-motivated) | 48 | Developed for this study |
| **Total** | **84** | |
| BG22 Levenshtein (reported separately) | 6 | Gaskell & Bowern (2022) |
...
Full tolerances for all 90 metrics (including the 6 Levenshtein metrics) are in the `TOLERANCES` dictionary of `score_85_metrics.py`.
...
### S3.3a Levenshtein Metrics (BG-42 Completion)

All 6 Levenshtein-distance metrics were computed for all 23 generators. VMS baseline values and tolerances:

| Metric | VMS value | Tolerance |
|---|---|---|
| `wordunique_mean` | 1.131 | ±0.100 |
| `wordunique_std` | 0.885 | ±0.050 |
| `wordunique_skew` | 3.947 | ±0.300 |
| `wordchange_mean` | 0.970 | ±0.100 |
| `wordchange_std` | 0.741 | ±0.050 |
| `wordchange_skew` | 3.609 | ±0.300 |

Pass rates by generator (ranked as above):

| Generator | Lev pass | /90 total | Failed metrics |
|---|---|---|---|
| Gen-Avoid | 4/6 | 75/90 | wordunique_skew, wordchange_skew |
| Gen-SP | 1/6 | 60/90 | all except wordchange_mean |
| Combined | 4/6 | 50/90 | wordunique_skew, wordchange_skew |
| Gen-09 | 4/6 | 50/90 | wordunique_skew, wordchange_skew |
| Gen-TS | 4/6 | 50/90 | wordunique_skew, wordchange_std |
| Gen-04T | 4/6 | 49/90 | wordunique_skew, wordchange_skew |
| Gen-04 | 3/6 | 47/90 | wordunique_skew, wordchange_skew, wordunique_std |
| Gen-06 | 4/6 | 46/90 | wordunique_skew, wordchange_skew |
| Section-Profiled | 4/6 | 45/90 | wordunique_skew, wordchange_skew |
| Gen-05 | 4/6 | 45/90 | wordunique_skew, wordchange_skew |
| Gen-08 | 4/6 | 45/90 | wordunique_skew, wordchange_skew |
| Gen-07 | 4/6 | 44/90 | wordunique_skew, wordchange_skew |
| P70C | 5/6 | 42/90 | wordunique_skew |
| Dual | 5/6 | 41/90 | wordunique_skew |
| Bigram | 1/6 | 40/90 | all except wordunique_skew |
| Gen-03 | 3/6 | 40/90 | wordunique_skew, wordchange_skew, wordunique_std |
| Gen-10 | 2/6 | 39/90 | wordunique_skew, wordchange_skew, wordunique_std, wordchange_std |
| Gen-SD | 4/6 | 38/90 | wordunique_skew, wordunique_std |
| Gen-00 | 1/6 | 32/90 | all except wordunique_mean |
| Gen-02 | 2/6 | 31/90 | wordunique_skew, wordchange_skew, wordunique_std, wordchange_std |
| Scribal | 0/6 | 29/90 | all 6 |
| Gen-0M | 2/6 | 25/90 | wordunique_skew, wordchange_skew, wordunique_std, wordchange_std |
| Gen-0W | 2/6 | 16/90 | wordunique_skew, wordchange_skew, wordunique_std, wordchange_std |

**Key finding:** `wordunique_skew` is universally failed — no generator comes within tolerance (closest: Gen-Avoid at 4.199 vs VMS 3.947, tolerance 0.300, miss by 0.25). This metric measures how asymmetrically distributed the edit-distance uniqueness values are across 200-word subsamples. The VMS's high skew indicates a few words are unusually distant from the rest of the vocabulary, a property none of the 23 tested production mechanisms reproduce.

The relative ordering of generators is unchanged by the Levenshtein addition. Gen-SP gains only 1 point (59→60/90) because it fails 5/6 Levenshtein metrics — its strength lies in the structural rather than the edit-distance domain. Gen-Avoid gains 4 points (71→75/90), consistent with its access to the actual VMS triple stream.
