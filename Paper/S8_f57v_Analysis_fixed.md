...
| 1 (innermost) | Line 5 | Mixed singles → words | Assembly demonstration |
| 2 | Line 4 | Standard word spacing | Full Voynichese |
| 3 | Line 3 | **Individually spaced glyphs** | Glyph catalogue (period-12) |
| 4 (outermost) | Line 2 | Standard word spacing | Full Voynichese |
| Margins | Lines 10–13 | N/A | Annotation labels |

Individual spacing in Ring 3 is confirmed in the Beinecke scan — it is not a transcription artefact.

---

## S8.2 Uniqueness of f57v

To establish that Ring 3's structure is not a generic feature of circular-text pages, all 26 pages in the manuscript containing circular text (@Cc/+Cc markers in ZLZI) were tested for individually-spaced glyph sequences.
...
## S8.3 Ring 3 (Line 3): The Glyph Catalogue

Ring 3 contains a period-12 sequence repeated four times with two variant positions:
...
The 25 EVA characters attested in the ZLZI corpus divide into two functionally distinct classes. Ring 3 enumerates one class and omits the other.

**Skeleton characters (13) — enumerated in Ring 3:**
...
**Dressing characters (6) — absent from Ring 3:**
...
The dressing characters are the **variable** part of Voynichese morphology: vowels and consonant clusters that fill and connect slots. Ring 3 enumerates the **fixed frame**.

---

## S8.5 Statistical Significance of the Partition

**Hypergeometric test.** The null hypothesis is that the 13 characters enumerated in Ring 3 are drawn uniformly at random from the 25 EVA characters. The probability of drawing exactly the 13 skeleton characters (and no dressing characters) when selecting 13:
...
The gallows parameters, by contrast, vary substantially across scribal hands (k/t ratio CV = 37%, f/p ratio CV = 35%), consistent with Ring 3's paradigm showing these as freely substitutable positions rather than fixed values.

---

## S8.7 Ring 1 (Line 5): Progressive Assembly

Ring 1 demonstrates a three-stage construction sequence:

| Stage | Positions | Content | Introduced |
|---|---|---|---|
| 1. Skeleton | 0–4 | `o v l r m` | Pure Ring 3 characters |
...
The missing-character introduction order (`a`, `i` → `s` → `e` → `c`, `h`) matches PGCS slot ordering (Prefix compounds before Core vowels before Suffix compounds). Ring 1 is a construction manual: skeleton characters first, dressing characters added progressively, compound prefixes introduced last.

---

## S8.8 Independence of the f57v Result

The skeleton/dressing partition was derived from the spatial layout of a single folio (f57v Ring 3). The PGCS slot assignments were derived from corpus-wide statistics of 37,465 tokens across 226 folios. The two analyses converge on the same 13/6 character partition and the same slot structure.
...
f57v is a structural reference for the Voynich notation system. The four rings present different levels of the writing system, with the character inventory at Ring 3 and progressive assembly at Ring 1 (innermost). Ring assignments follow Stolfi's (1998) convention reversed (Ring 1 = innermost), confirmed against the Beinecke scan and ZLZI line mappings:

- **Ring 1 (innermost, Line 5):** Demonstrates progressive assembly — skeleton → dressing → full tokens
- **Ring 2 (Line 4):** Fully assembled Voynichese in normal operation
- **Ring 3 (Line 3):** Enumerates the 13 fixed-frame characters with a paradigmatic gallows substitution table
- **Ring 4 (outermost, Line 2):** Fully assembled Voynichese in normal operation
