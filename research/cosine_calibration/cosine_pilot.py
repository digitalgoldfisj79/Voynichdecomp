...
def fetch_candidate_crop(sess, rec):
    r = http_get(sess, rec['search_url'], headers={'User-Agent':'ManuComp-CosineCalibration/0.2'}, timeout=45)
...
def main():
    ...
    common = [r for r in completed if r.get('work_id') in sobel_ranks]
    if common:
        a = np.array([sobel_ranks[r['work_id']] for r in common], dtype=np.float64)
        b = np.array([cosine_ranks[r['work_id']] for r in common], dtype=np.float64)
        rank_corr = float(np.corrcoef(a,b)[0,1]) if len(common) > 1 else None
    else:
        rank_corr = None

    out = {
        'version':'cosine-calibration-v0.2-classical',
