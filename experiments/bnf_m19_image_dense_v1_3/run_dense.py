...
ns={'__name__':'arm_a_dense_lib'};exec(compile(src,'run_arm_a.py','exec'),ns)

def load_image_data_dense():
    sel=[];folios=set();path=os.path.join(ns['DATA'],'corpus_crop_manifest.jsonl')
    with open(path) as h:
    ...
    z=np.load(os.path.join(ns['DATA'],'corpus_embeddings_full_dense.npz'),allow_pickle=False);ids=z['ids'];checks=np.linspace(0,len(sel)-1,min(1000,len(sel)),dtype=int)
    for j in checks:
        q=sel[j]
        if ids[q[0]]!=q[1]+'::norm':raise RuntimeError(('dense manifest/vector order mismatch',j,ids[q[0]],q[1]))
    print('DENSE_VECTOR_ARCHIVE',len(ids),z['vectors'].shape,str(z['kind']),flush=True)
    ...
    print('DENSE_IMAGE_CENSUS',json.dumps({'rows':len(sel),'folios':len(folios),'T':len(T),'H':len(H),'C':len(C),'Tfit':len(split['Tf']),'Tvis':len(split['Tv'])},separators=(',',':')),flush=True)
    return X,rec,split

ns['load_image_data']=load_image_data_dense
ns['main']()
