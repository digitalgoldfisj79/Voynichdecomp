...
def load_language(repo:Path,iso:str):
    root=repo/"experiments"/"recoverability_frontier_v0_5"; languages=svt.core.load_languages(root/"corpus_manifest_v050.json",repo/".cache"/"svt-v011")
...
def main()->None:
    ...
    config=Path(__file__).resolve().parent/"FROZEN_CONFIG_v0_1.json"; payload={"programme":"stateful_verbose_transducer_v0_1_1","stage":args.stage,"split":args.split,"iso":args.iso,"length":args.length,"replicates_per_mode":args.replicates,"config_sha256":sha256_file(config),"rows":rows,"controls":controls,"summary":summary}; locked=False
