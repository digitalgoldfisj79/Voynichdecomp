...
ns={'__name__':'v09lib'}
exec(compile(src,'run_v09.py','exec'),ns)
b=ns['b']
...
def main():
    lms,pools,meta=ns['load_fresh']();data=json.loads(b['fetch'](b['SLIM']));sample,hold,pages,required=ns['split_vms'](data)
    out={'heldout_folios':[f for f,_,_ in hold],'mapping':RAW,'results':{}}
    # ZLZI lexical only
    zw=words_for(data,hold,'ZLZI');zlex=ns['lexical_z'](zw,M,SYMS,lms['german'],('postsel','ZLZI','german'));out['results']['ZLZI']={'coverage':exact_coverage(zw)[0],'lexical':zlex};print('ZLZI',json.dumps(out['results']['ZLZI'],separators=(',',':')),flush=True)
    for tid in ['TTLI','VDRB']:
        words=words_for(data,hold,tid);cov,known,total=exact_coverage(words);rank=[]
        for la in b['LANGS']:
            sc,n,sk,fcov=ns['forward_words'](words,M,SYMS,lms[la]);rank.append((la,sc))
        rank.sort(key=lambda x:x[1],reverse=True);gscore=next(x[1] for x in rank if x[0]=='german');grank=1+next(i for i,x in enumerate(rank) if x[0]=='german');margin=(rank[0][1]-rank[1][1]) if rank[0][0]=='german' else None;lex=ns['lexical_z'](words,M,SYMS,lms['german'],('postsel',tid,'german'))
