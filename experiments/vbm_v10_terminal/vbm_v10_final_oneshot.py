...
import json, math, runpy, time, urllib.request
...
# 1) Mandatory in-job software smoke; any failure aborts before binding fits.
...
# 2) Load frozen exact-block GPU solver, applying only the pre-existing execution patch 3.
...
G={'__name__':'v10gpu'}
exec(compile(src,'vbm_v10_stage_a_gpu_positive_patched3.py','exec'),G)
B=G['B']
...
def one_o2(lang,rep,A,all_lines,key):
    ...
    r.update({
        'HOLD_LM':hl,
        'RAND_HOLD_LM':base,
        'RAND_HOLD_SD':base_sd,
        'HOLD_ADV':hl-base,
        'FIT_LM':float(m['fit_score']),
    ...
    return {
        'lang':lang,'rep':rep,'size':SIZE,'fit_lines':len(fit),'hold_lines':len(hold),
...
def gate(rows):
    ...
    for la in ['DE','IT']:
        ...
        by_lang[la]={
            'char_pass':sum(x['REC_CHAR']>=.80 for x in q),
...
def main():
    ...
    print('V10_FINAL_META='+json.dumps({
        'protocol':'VBM_V10_FINAL_ONESHOT_PROTOCOL.md','size':SIZE,'gpus':torch.cuda.device_count(),
        'devices':[torch.cuda.get_device_name(i) for i in range(torch.cuda.device_count())],
        'bridge_block':G['BRIDGE_BLOCK'],'nucleus_block':G['NUC_BLOCK'],'sweeps':G['SWEEPS'],'chains':G['CHAINS']
    ...
    g=gate(rows)
    verdict=('VBM_COMPACT_RECOVERY_GATE_PASSED_FULL_TAIL_REQUIRED' if g['PASS']
             else 'VBM_GLOBAL_KEY_NOT_RECOVERABLE_EVEN_COMPACT')
    out={'verdict':verdict,'gate':g,'elapsed_s':time.time()-t0,
         'full_tail_opened':False,'voynich_opened':False}
    print('VBM_V10_FINAL_RESULT='+json.dumps(out,sort_keys=True),flush=True)
