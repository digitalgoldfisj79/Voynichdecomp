#!/usr/bin/env python3
"""Stage 4 v2: deterministic blind object triage for the sealed IIIF subset.

Pre-outcome parser repair: Qwen may return either {"objects": [...]} or a
bare JSON list. Both are normalized to the same schema before mechanical QA.
No VMS images or similarity are loaded.
"""
...
def select(key,pages):
 ...
 if key=='external:bodl_canon_misc_554':
  z=[]
  for p in usable:
   if re.search(r'(?<!\d)(15[4-9]|16\d|17[0-4])\s*[rv]?',p['label']): z.append(p)
...
def main():
 ...
 print('STAGE4_CONFIG='+json.dumps({'model':MODEL,'sources':len(SOURCES),'classes':CLASSES,'seed':SEED,'parser':'v2-list-or-dict'}),flush=True)
 for key,mu in SOURCES.items():
  ...
  for p in chosen:
   ...
   try:
    ...
    try:data=parse_json_answer(ans); rawobjs=normalize_objects(data)
    except Exception:data={'parse_error':ans[:500]};rawobjs=[]
    valid=[]
    for o in rawobjs:
     ...
     gf=o.get('geometry_features') or {}
     if not isinstance(gf,dict): gf={'raw':gf}
     valid.append({'object_class':cls,'bbox_1000':[x0,y0,x1,y1],'description':str(o.get('description',''))[:700],'geometry_features':gf})
