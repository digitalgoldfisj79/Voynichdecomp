...
def run(cipher,q,u,pu,name):
 ...
 for r in range(72):
  sd=m.seedint(f'TRANCHSTA21TFIT::{name}::{r}');rng=np.random.default_rng(sd)
  if r==0:k=init.copy()
  ...
  if (r+1)%6==0:hist.append([r+1,bs]);print('V21_TFIT',name,r+1,bs,flush=True)
 return bk,bs,hist
def main():
 a=argparse.ArgumentParser();a.add_argument('ensemble');x=a.parse_args();q,u,pu=model();fol,vocab,ids,T,H,C=m.parse_rf();seg,meta,cov,_,_=m.retained_segments(fol,T,ids);cipher=m.flatten(seg);k,s,h=run(cipher,q,u,pu,x.ensemble);freq=np.bincount(cipher[cipher>=0],minlength=36);print('V21_TFIT_RESULT='+__import__('json').dumps({'ensemble':x.ensemble,'score':s,'key':k.tolist(),'history':h,'coverage':cov,'freq':freq.tolist()},separators=(',',':')))
