#!/usr/bin/env python3
"""Authorised locked-test run for frozen v0.6 Family P.

This is identical to the development solver and uses only the previously frozen
seed-generation termination correction. No test-dependent tuning is permitted.
"""
...
def main() -> None:
    ...
    languages = core.load_languages(
        ...
        repo / ".cache" / "v060-family-p2-test-preflight",
    )
    language = languages["en"]
    trial = blind.make_trial(language, "test", 384, "periodic", 0)
    preflight = []
    ...
    print("V060_P2_TEST_SEED_PREFLIGHT", json.dumps(preflight, sort_keys=True), flush=True)
    ...
    payload["execution_clarification"] = {
        ...
        "development_gate_commit": "0dd4a5121e9e8070037ccd5760c821f123a58f12",
    }
    print("V060_P2_TEST_FULL_PAYLOAD", json.dumps(payload, sort_keys=True), flush=True)
