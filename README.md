...
The validation script also runs 19 alternative decompositions (conventional parses, boundary shifts, random splits, fixed-position cuts, ablated inventories) and shows that none comes within 1 bit of the grammar's entropy profile. The nearest non-degenerate alternative is 1,074× further from the target.
...
**Category 2: Boundary perturbation (1)**
...
Tests whether P70's ch/sh boundary is in the right place by splitting the digraph differently.

**Category 3: Structural variants (2)**
...
**Category 4: Fixed-position splits (6)**

| Alternative | Description |
|------------|-------------|
| Fixed(1,1) | First 1 char = prefix, last 1 = suffix |
| Fixed(1,2) | First 1 char = prefix, last 2 = suffix |
| Fixed(1,3) | First 1 char = prefix, last 3 = suffix |
| Fixed(2,1) | First 2 chars = prefix, last 1 = suffix |
| Fixed(2,2) | First 2 chars = prefix, last 2 = suffix |
| Fixed(2,3) | First 2 chars = prefix, last 3 = suffix |

These are deliberately "dumb" baselines that segment by character position only.

**Category 5: Randomised baselines (8)**

| Alternative | Description | Key difference |
|------------|-------------|----------------|
| Random × 5 | Random character boundaries for each token (seeds 42–46) | Null model |
| Grammar P/G + random suffix × 3 | Keep P70's prefixes and gallows, randomise suffixes (seeds 200–202) | Tests suffix inventory specifically |
