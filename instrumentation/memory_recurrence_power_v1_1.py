...
import memory_recurrence_score_v1_1 as score
...
def index_power(objs):
    ...
    for o in objs:
        ...
        if key in d and json.dumps(d[key],sort_keys=True)!=json.dumps(o,sort_keys=True): raise ValueError(f'conflicting duplicate {key}')
        d[key]=o
...
def main():
    ...
    out={'assay_id':ASSAY,'n_per_cell':N,'scorer':'memory_recurrence_score_v1_1.py','by_representation':{}}
    for rep in REPS:
        fams={}
        for src,grid in GRIDS.items():
            cells=[]
